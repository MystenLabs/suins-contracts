...
module suins::auction_tests_3 {
    ...
    fun test_place_bid_aborts_if_exists() {
        ...
        place_bid_util(scenario, seal_bid, 3300 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 10);
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 2000 * suins::constants::mist_per_sui(), SECOND_SECRET);
        place_bid_util(scenario, seal_bid, 12200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 10);
    ...
    fun test_bids_multiple_times_same_domain_and_same_highest_value_then_withdraw() {
        ...
        place_bid_util(scenario, seal_bid, 3300 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 10);
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 2000 * suins::constants::mist_per_sui(), SECOND_SECRET);
        place_bid_util(scenario, seal_bid, 12200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::some(FIRST_TX_HASH), 10);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let ctx = ctx_new(
                FIRST_USER_ADDRESS,
                SECOND_TX_HASH,
                START_AN_AUCTION_AT + 200 * suins::constants::mist_per_sui(),
                20
            );
            let bids = get_bids_by_bidder(&auction, FIRST_USER_ADDRESS);
            assert!(vector::length(&bids) == 2, 0);
    ...
    fun test_bids_multiple_times_same_domain_and_same_highest_value_then_withdraw_2() {
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let suins = test_scenario::take_shared<SuiNS>(scenario);
            let coin = coin::mint_for_testing<SUI>(12222 * suins::constants::mist_per_sui() + BIDDING_FEE, &mut ctx);
            auction::place_bid(&mut auction, &mut suins, seal_bid, 12200 * suins::constants::mist_per_sui(), &mut coin, &clock, &mut ctx);
            assert!(coin::value(&coin) == 22 * suins::constants::mist_per_sui(), 0);
    ...
    fun test_bids_multiple_times_same_domain_and_same_highest_value_then_withdraw_3() {
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let auction = test_scenario::take_shared<AuctionHouse>(scenario);
            let clock = test_scenario::take_shared<Clock>(scenario);
            let coin = coin::mint_for_testing<SUI>(3301 * suins::constants::mist_per_sui() + BIDDING_FEE, &mut ctx);
            let suins = test_scenario::take_shared<SuiNS>(scenario);
            auction::place_bid(&mut auction, &mut suins, seal_bid, 3300 * suins::constants::mist_per_sui(), &mut coin, &clock, &mut ctx);
            assert!(coin::value(&coin) == suins::constants::mist_per_sui(), 0);
            coin::burn_for_testing(coin);
            test_scenario::return_shared(suins);
            test_scenario::return_shared(auction);
            test_scenario::return_shared(clock);
    ...
    fun test_bids_multiple_times_same_domain_and_same_highest_value_but_not_yet_reveal_then_withdraw() {
        ...
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 2000 * suins::constants::mist_per_sui(), SECOND_SECRET);
        place_bid_util(scenario, seal_bid, 12200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 20);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            assert!(coin::value(&coin2) == 3300 * suins::constants::mist_per_sui(), 0);
    ...
    fun test_reveal_bid_handle_invalid_bid_if_place_bid_too_early() {
        ...
        start_an_auction_util(scenario, FIRST_DOMAIN_NAME);
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 1100 * suins::constants::mist_per_sui(), FIRST_SECRET);
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            let ctx = ctx_new(
                FIRST_USER_ADDRESS,
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            reveal_bid_util(
                ...
                START_AN_AUCTION_AT + 1 + BIDDING_PERIOD,
                FIRST_DOMAIN_NAME,
                1100 * suins::constants::mist_per_sui(),
                FIRST_SECRET,
                FIRST_USER_ADDRESS,
                5
            );
    ...
    fun test_reveal_bid_handle_invalid_bid_if_place_bid_too_late() {
        ...
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 11100 * suins::constants::mist_per_sui(), FIRST_SECRET);
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            let ctx = ctx_new(
                FIRST_USER_ADDRESS,
            ...
            let suins = test_scenario::take_shared<SuiNS>(scenario);
            let clock = test_scenario::take_shared<Clock>(scenario);
            let coin = coin::mint_for_testing<SUI>(30000 * suins::constants::mist_per_sui() + BIDDING_FEE, ctx);

            assert!(auction::get_balance(&auction) == 0, 0);
            auction::place_bid(&mut auction, &mut suins, seal_bid, 30000 * suins::constants::mist_per_sui(), &mut coin, &clock, ctx);
            assert!(auction::get_balance(&auction) == 30000 * suins::constants::mist_per_sui(), 0);
            assert!(coin::value(&coin) == 0, 0);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            reveal_bid_util(
                ...
                START_AN_AUCTION_AT + 1 + BIDDING_PERIOD,
                FIRST_DOMAIN_NAME,
                11100 * suins::constants::mist_per_sui(),
                FIRST_SECRET,
                FIRST_USER_ADDRESS,
                5
            );
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let coin = test_scenario::take_from_address<Coin<SUI>>(scenario, FIRST_USER_ADDRESS);
            assert!(coin::value(&coin) == 30000 * suins::constants::mist_per_sui(), 0);
            let (start_at, highest_bid, second_highest_bid, winner, is_finalized) =
                auction::get_entry(&auction, utf8(FIRST_DOMAIN_NAME));
            assert!(option::extract(&mut start_at) == START_AN_AUCTION_AT + 1, 0);
            assert!(option::extract(&mut highest_bid) == 0, 0);
    ...
    fun test_reveal_bid_handle_second_highest_bid() {
        ...
        start_an_auction_util(scenario, FIRST_DOMAIN_NAME);
        place_bid_util(scenario, seal_bid, 4100 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 10);
        test_scenario::next_tx(scenario, SUINS_ADDRESS);
        {
            let auction = test_scenario::take_shared<AuctionHouse>(scenario);
            let coin = test_scenario::most_recent_id_for_address<Coin<SUI>>(FIRST_USER_ADDRESS);
        ...
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, SECOND_USER_ADDRESS, 1500 * suins::constants::mist_per_sui(), FIRST_SECRET);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            assert!(vector::length(&ids) == 1, 0);
            let coin = test_scenario::take_from_address<Coin<SUI>>(scenario, FIRST_USER_ADDRESS);
            assert!(coin::value(&coin) == 2600 * suins::constants::mist_per_sui(), 0);
            test_scenario::return_to_address(FIRST_USER_ADDRESS, coin);
        };
        ...
        test_scenario::next_tx(scenario, SECOND_USER_ADDRESS);
        {
            ...
            get_entry_util(&mut auction,
                FIRST_DOMAIN_NAME, START_AN_AUCTION_AT + 1, 3000 * suins::constants::mist_per_sui(), 1500 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, true);
            test_scenario::return_shared(auction);
        };
        test_scenario::end(scenario_val);
    }
    ...
    fun test_reveal_bid_handle_low_value_bid() {
        ...
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 3000 * suins::constants::mist_per_sui(), FIRST_SECRET);
        start_an_auction_util(scenario, FIRST_DOMAIN_NAME);
        place_bid_util(scenario, seal_bid, 4100 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 15);
        test_scenario::next_tx(scenario, SUINS_ADDRESS);
        {
            let auction = test_scenario::take_shared<AuctionHouse>(scenario);
            let coin = test_scenario::most_recent_id_for_address<Coin<SUI>>(FIRST_USER_ADDRESS);
        ...
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, THIRD_USER_ADDRESS, 1200 * suins::constants::mist_per_sui(), FIRST_SECRET);
        place_bid_util(scenario, seal_bid, 2000 * suins::constants::mist_per_sui(), THIRD_USER_ADDRESS, 0, option::some(SECOND_TX_HASH), 10);
        ...
        test_scenario::next_tx(scenario, SECOND_USER_ADDRESS);
        {
            let auction = test_scenario::take_shared<AuctionHouse>(scenario);
            get_entry_util(&mut auction,
                FIRST_DOMAIN_NAME, START_AN_AUCTION_AT + 1, 3000 * suins::constants::mist_per_sui(), 1500 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, false);
            test_scenario::return_shared(auction);
        };
        ...
        test_scenario::next_tx(scenario, SECOND_USER_ADDRESS);
        {
            ...
            assert!(coin::value(&first_coin) == 2000 * suins::constants::mist_per_sui(), 0);
            let second_coin = test_scenario::take_from_address<Coin<SUI>>(scenario, SECOND_USER_ADDRESS);
            assert!(coin::value(&second_coin) == 75 * suins::constants::mist_per_sui(), 0);
    ...
    fun test_finalize_bid_handle_being_called_twice_by_non_winner() {
        ...
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, SECOND_USER_ADDRESS, 2000 * suins::constants::mist_per_sui(), FIRST_SECRET);
        place_bid_util(scenario, seal_bid, 3300 * suins::constants::mist_per_sui(), SECOND_USER_ADDRESS, 0, option::some(FIRST_TX_HASH), 10);
        test_scenario::next_tx(scenario, SECOND_USER_ADDRESS);
        {
            let auction = test_scenario::take_shared<AuctionHouse>(scenario);
            let suins = test_scenario::take_shared<SuiNS>(scenario);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            assert!(coin::value(&second_coin) == 1100 * suins::constants::mist_per_sui(), 0);

            let ids = test_scenario::ids_for_address<Coin<SUI>>(SECOND_USER_ADDRESS);
            assert!(vector::length(&ids) == 1, 0);

            let coin = test_scenario::take_from_address<Coin<SUI>>(scenario, SECOND_USER_ADDRESS);
            assert!(coin::value(&coin) == 2300 * suins::constants::mist_per_sui(), 0);
            test_scenario::return_to_address(SECOND_USER_ADDRESS, coin);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let first_coin = test_scenario::take_from_address<Coin<SUI>>(scenario, FIRST_USER_ADDRESS);
            assert!(coin::value(&first_coin) == 1100 * suins::constants::mist_per_sui(), 0);
            let second_coin = test_scenario::take_from_address<Coin<SUI>>(scenario, FIRST_USER_ADDRESS);
            assert!(coin::value(&second_coin) == 50 * suins::constants::mist_per_sui(), 0);
    ...
    fun test_bids_multiple_domains_and_reveal_only_one() {
        ...
        let seal_bid = make_seal_bid(SECOND_DOMAIN_NAME, FIRST_USER_ADDRESS, 2000 * suins::constants::mist_per_sui(), FIRST_SECRET);
        place_bid_util(scenario, seal_bid, 2200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 10);
        let seal_bid = make_seal_bid(THIRD_DOMAIN_NAME, FIRST_USER_ADDRESS, 3000 * suins::constants::mist_per_sui(), FIRST_SECRET);
        place_bid_util(scenario, seal_bid, 12200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 10);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            let auction = test_scenario::take_shared<AuctionHouse>(scenario);
            let suins = test_scenario::take_shared<SuiNS>(scenario);
            assert!(auction::get_balance(&auction) == 15700 * suins::constants::mist_per_sui(), 0);
            ...
            let (bidder, mask, created_at, is_unsealed) = get_bid_detail_fields(bid_detail);
            assert!(bidder == FIRST_USER_ADDRESS, 0);
            assert!(mask == 2200 * suins::constants::mist_per_sui(), 0);
            assert!(created_at == START_AN_AUCTION_AT + 1, 0);
            assert!(!is_unsealed, 0);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let first_coin = test_scenario::take_from_address<Coin<SUI>>(scenario, FIRST_USER_ADDRESS);
            assert!(coin::value(&first_coin) == 200 * suins::constants::mist_per_sui(), 0);
            assert!(auction::get_balance(&auction) == 13500 * suins::constants::mist_per_sui(), 0);
            assert!(
                suins::balance(&suins) == 3 * START_AN_AUCTION_FEE + 2000 * suins::constants::mist_per_sui() + BIDDING_FEE * 3,
                0,
            );
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            assert!(coin::value(&coin3) == 200 * suins::constants::mist_per_sui(), 0);
            test_scenario::return_to_address(FIRST_USER_ADDRESS, coin1);
            test_scenario::return_to_address(FIRST_USER_ADDRESS, coin2);
    ...
    fun test_bids_multiple_times_same_domain_and_reveal_only_one_bid() {
        ...
        place_bid_util(scenario, seal_bid, 12200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 20);
        let seal_bid = make_seal_bid(FIRST_DOMAIN_NAME, FIRST_USER_ADDRESS, 3000 * suins::constants::mist_per_sui(), FIRST_SECRET);
        place_bid_util(scenario, seal_bid, 10200 * suins::constants::mist_per_sui(), FIRST_USER_ADDRESS, 0, option::none(), 25);
        ...
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
            ...
            let ctx = ctx_new(
                FIRST_USER_ADDRESS,
                x"3a985da74fe225b2045c172d6bd390bd855f086e3e9d525b46bfe24511431532",
                START_AUCTION_END_AT + 200 * suins::constants::mist_per_sui(),
                10
            );
            let bids = get_bids_by_bidder(&auction, FIRST_USER_ADDRESS);
            assert!(vector::length(&bids) == 3, 0);
            ...
            let bids = get_bids_by_bidder(&auction, FIRST_USER_ADDRESS);
            assert!(vector::length(&bids) == 1, 0);
            assert!(auction::get_balance(&auction) == 10200 * suins::constants::mist_per_sui(), 0);
            test_scenario::return_shared(auction);
        };
        test_scenario::next_tx(scenario, FIRST_USER_ADDRESS);
        {
