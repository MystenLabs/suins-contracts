...
export class PriceServiceConnection {
	 ...
	 */
	async getVaa(priceId: HexString, publishTime: EpochTimeStamp): Promise<[string, EpochTimeStamp]> {
	 ...
	 * @returns PriceFeed
	 */
	async getPriceFeed(priceId: HexString, publishTime: EpochTimeStamp): Promise<PriceFeed> {
	 ...
	 */
	async subscribePriceFeedUpdates(priceIds: HexString[], cb: PriceFeedUpdateCallback) {
	 ...
	 */
	async unsubscribePriceFeedUpdates(priceIds: HexString[], cb?: PriceFeedUpdateCallback) {
		if (this.wsClient === undefined) {
			await this.startWebSocket();
		}
