...
export const createNodeSubdomain = async (name: String, parent: string) => {
    ...
    let node = txb.moveCall({
        target: `${config.subdomainsPackageId}::subdomains::create`,
        arguments: [
    ...
    txb.moveCall({
        target: `0x9af70a4cb6d7144e68fd972eef672a74c7fe41aa5c0bb67ba40d7d1ae87bfb19::direct_setup::set_target_address`,
        arguments: [
...
export const createLeafSubdomainTx = (txb: TransactionBlock, name: string, parent: string, config: PackageInfo) => {
   // create a leaf subdomain
   txb.moveCall({
    target: `${config.subdomainsPackageId}::subdomains::create_leaf`,
    arguments: [
        txb.object(config.suins), // suins Object
...
export const createLeafSubdomain = async (name: string, parent: string) => {
    ...
    createLeafSubdomainTx(txb, name, parent, config);
 
    await executeTx(signer, txb);
}

export const removeLeafSubdomain = async (name: string, parent: string) => {
    const txb = new TransactionBlock();
...
const prepareManyNamesForTesting = async (startInt=0) => {
    ...
    for(let i = startInt;i<startInt + 900; i++){
        ...
        createLeafSubdomainTx(txb, name, MANOS_SUI_OBJECT_ID, config);
    }
...
// createNodeSubdomain('example.manos.sui', MANOS_SUI_OBJECT_ID);
