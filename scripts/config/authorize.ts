...
export const authorizeApp = async (network: Network, txb: TransactionBlock, app: string) => {
    ...
    txb.moveCall({
        ...
        arguments: [
          txb.object(suinsPackageConfig.adminCap),
          txb.object(suinsPackageConfig.suins),
        ],
        typeArguments: [app],
    });
}
...
export const deauthorizeApp = async (network: Network, txb: TransactionBlock, app: string): Promise<TransactionBlock | void> => {
    const suinsPackageConfig = mainPackage[network];
