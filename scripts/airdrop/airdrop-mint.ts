...
const prepareCoinObjects = async (chunks: number) => {
	...
	for (let i = 0; i < chunks; i++) {
		const coin = tx.splitCoins(tx.gas, [tx.pure(3_100_000_000, 'u64')]);
	...
	return res?.objectChanges
		?.filter((x) => x.type === 'created' && x.objectType === SUI_COIN_TYPE)
		.map((x: SuiObjectData) => ({
			objectId: x.objectId,
			version: x.version,
			digest: x.digest,
...
const mintDayOne = async ({
	...
	tx.moveCall({
		...
		arguments: [tx.sharedObjectRef(config.dropListObj), buffer],
	});
...
const executeMintsForBatches = async (batches: string[][], initialBatch = 0) => {
	...
	while (currentSliceStart < batches.length) {
		...
		const results = await executeConcurrently(batchToExecute, {
	...
	console.log(
		`Successfully run ${success} batches, minting ${
			success * MAX_MINTS_PER_TRANSACTION
		} DayOne objects.`,
	);
	console.log(failedChunks);
	console.log(
		`Failed to execute ${failedChunks.length} or ${
			failedChunks.length * MAX_MINTS_PER_TRANSACTION
		} mints.`,
	);
};

const executeConcurrently = async (
	slicedBatches: string[][],
	options: {
		sliceStart: number;
		failedChunks: number[];
	},
