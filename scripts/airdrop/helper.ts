...
export const executeTx = async (
	...
	return signer
		...
		.then(function (res) {
			...
			if (options?.isAirdropExecution) {
				if (getExecutionStatus(res)?.status === 'success')
					console.log(`Success of chunk: ${options?.chunkNum}`);
				else {
...
export const readAddressesFromFile = (
	fileNameWithPath: string = 'airdrop_addresses.txt',
): string[] => {
...
export const csvToBatches = (csvAddressList: string[]): string[][] => {
...
export const batchToHash = (batch: string[]) => {
	...
	return blake2.createHash('blake2b', { digestLength: 32 }).update(bytes).digest('hex');
};
...
export const prepareSigner = (provider: JsonRpcProvider): RawSigner => {
	...
	if (!phrase)
		throw new Error(
			`ERROR: Admin mnemonic is not exported! Please run 'export ADMIN_PHRASE="<mnemonic>"'`,
		);
...
export const prepareSignerFromPrivateKey = (network: Network) => {
	...
	if (!privateKey)
		throw new Error(
			`ERROR: Private key not exported or exported wrong! Please run 'export PRIVATE_KEY="<mnemonic>"'`,
		);
...
export const addressesToBuffer = (
	tx: TransactionBlock,
	batch: string[],
...
export const prepareMultisigTx = async (tx: TransactionBlock, network: Network) => {
	...
	tx.build({
		...
		const output_location =
			process.env.NODE_ENV === 'development' ? './tx-data-local.txt' : '$PWD/tx/tx-data.txt';
