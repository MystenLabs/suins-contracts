...
Metadata is strongly recommended for any application. The appCap must be provided, along with a key/value pair.
Key should be `description`, `icon_url`, `documentation_url`, `homepage_url`, or `contact`.
