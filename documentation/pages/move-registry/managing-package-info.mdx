...
You can add source code information on your `PackageInfo` object by the following setup.

To update the source for a version, you should first call 
...
## Removing / updating source code information
...
// .. you could then follow the steps above to add the new source code info for that version.
``` 
...
build custom access-control for your object, you can utilize TTO (Transfer-to-Object).

`PackageInfo` does not allow public transfers, in order to ensure that the object is always indexable 
