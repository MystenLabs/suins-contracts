...
/// The coupon's format that is being generated is calculated based on
/// hash(discord_id + claim_idx)
...
/// authorization struct, to allow the app to create coupons in the coupon
/// system.
...
public struct Member has store, drop {
    /// Available points that can be converted into coupon codes. 1 point -> 1%
    /// discount.
    available_points: u64,
    /// Roles already assigned to a member. Helps us verify we never give the
    /// same rewards twice.
    roles: VecSet<u8>,
...
/// Can be called by anyone, only works for the { discord_id -> roles[] }
/// mapping.
...
/// Can be called only by anyone, only works for a specific pair (address ->
/// discordId)
...
/// Only claimable if there's a mapping {discord_id -> address (which needs to
/// be the sender)}
...
fun add_roles_internal(
    ...
    while (!roles.is_empty()) {
        ...
        // add discount to the member's available points. We can cast u8 to u64
        // safely.
        member.available_points =
            member.available_points + (discord_roles[&role] as u64);
    };
}
...
/// An internal coupon claim handler that validates data, creates the coupon
/// code, updates membership details
