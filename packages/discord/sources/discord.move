...
module discord::discord {
    ...
    public struct DiscordCap has key, store {
        id: UID
    }

    public struct AddressMapping has store { 
        updates: u32,
        addr: address
    }
    ...
    public struct Discord has key, store {
        id: UID,
    ...
    public fun attach_roles(discord: &mut Discord, signature: vector<u8>, discord_id: String, roles: vector<u8>) {
        ...
        if (!discord.users.contains(discord_id)) discord.users.add(discord_id, Member {
            available_points: 0,
            roles: vec_set::empty(),
    ...
    public fun set_address(discord: &mut Discord, signature: vector<u8>, discord_id: String, addr: address) {
        ...
        let updates_count = if (discord.address_mapping.contains(discord_id)) {
            let entry_mut = &mut discord.address_mapping[discord_id];
            entry_mut.addr = addr;
            entry_mut.updates = entry_mut.updates + 1;
            entry_mut.updates
        } else {
            discord.address_mapping.add(discord_id, AddressMapping {
                updates: 0,
                addr
            });
            0
    ...
    public(package) fun claim_coupon_internal(discord: &mut Discord, discord_id: String, amount: u8, ctx: &TxContext): String {
        ...
        assert!(discord.address_mapping.contains(discord_id), EDiscordIdNotFound);
        assert!(discord.users.contains(discord_id), EDiscordIdNotFound);

        // Find the user mapped to that discord id.
        let mapping = &discord.address_mapping[discord_id];
        assert!(mapping.addr == ctx.sender(), EAddressNoMapping);
