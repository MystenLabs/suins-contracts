...
fun regular_renewal_5_years() {
    ...
    assert!(
        nft.expiration_timestamp_ms() == clock.timestamp_ms() + (6 * year_ms()) - 10,
        0,
    );
...
fun failed_record_id_missmatch() {
    ...
    let clock = clock::create_for_testing(&mut ctx);
    let mut nft = nft::new_for_testing(
        domain::new(utf8(DOMAIN_NAME)),
        1,
        &clock,
        &mut ctx,
    );
...
fun failed_not_enough_money() {
    ...
    renewal::renew(
        ...
        coin::mint_for_testing<SUI>(
            (1 as u64) * REGULAR_PRICE * mist_per_sui(),
            &mut ctx,
        ),
        &clock,
    );
...
public fun renew_util(
    ...
    renewal::renew(
        ...
        no_years,
        coin::mint_for_testing<SUI>(
            (no_years as u64) * REGULAR_PRICE * mist_per_sui(),
            ctx,
        ),
        clock,
    );
