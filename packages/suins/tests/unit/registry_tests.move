...
fun override_leaf_record_after_change_of_parent_owner() {
    ...
    registry.add_leaf_record(
        domain::new(utf8(b"test2.test.sui")),
        &clock,
        @0x0,
        &mut ctx,
    );
...
#[test]
/// `burn_registration_object` burns the SuinsRegistration object as well as
/// removes the record,
...
fun burn_expired_subdomain() {
    ...
    let nft = registry.add_record(
        domain::new(utf8(b"node.test.sui")),
        1,
        &clock,
        &mut ctx,
    );
