...
#[
    test,
    expected_failure(
        abort_code = ::subdomains::subdomains::EInvalidExpirationDate,
    ),
]
...
#[
    test,
    expected_failure(
        abort_code = ::subdomains::subdomains::ECreationDisabledForSubDomain,
    ),
]
...
#[
    test,
    expected_failure(
        abort_code = ::subdomains::subdomains::EExtensionDisabledForSubDomain,
    ),
]
...
fun tries_to_extend_while_parent_changed() {
    ...
    let _parent_w_different_owner = create_sld_name(
        utf8(b"test.sui"),
        scenario,
    );
...
/// Get the active registry of the current scenario. (mutable, so we can add
/// extra names ourselves)
