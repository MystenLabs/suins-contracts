...
/// (E.g., with `.move` service, we might want to restrict how subdomains are
/// created)
...
/// Validate that the subdomain label (e.g. `sub` in `sub.example.sui`) is
/// valid.
...
public fun is_valid_label(domain: &Domain, config: &SubDomainConfig): bool {
    // our label is the last vector element, as labels are stored in reverse
    // order.
