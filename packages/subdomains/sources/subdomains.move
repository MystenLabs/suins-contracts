...
///
/// This module is responsible for creating subdomains and managing their
/// settings.
///
/// It allows the following functionality:
///
/// 1. Registering a new subdomain as a holder of Parent NFT.
/// 2. Setup the subdomain with capabilities (creating nested names, extending
/// to parent's renewal time).
...
/// Comments:
///
/// 1. By attaching the creation/extension attributes as metadata to the
/// subdomain's NameRecord, we can easily
/// turn off this package completely, and retain the state on a different
/// package's deployment. This is useful
/// both for effort-less upgradeability and gas savings.
/// 2. For any `registry_mut` call, we know that if this module is not
/// authorized, we'll get an abort
...
/// Tries to create a subdomain that expires later than the parent or below the
/// minimum.
...
/// Tries to extend the expiration of a subdomain which doesn't have the
/// permission to do so.
...
/// Parent for a given subdomain has changed, hence time extension cannot be
/// done.
...
public fun new_leaf(
    ...
    internal_validate_nft_can_manage_subdomain(
        suins,
        parent,
        clock,
        subdomain,
    ...
    // Aborts with `suins::registry::ERecordExists` if the subdomain already
    // exists.
    registry_mut(suins).add_leaf_record(subdomain, clock, target, ctx)
}

/// Removes a `leaf` subdomain from the registry.
/// Management of the `leaf` subdomain can only be achieved through the parent's
/// valid NFT.
public fun remove_leaf(
    ...
    // All validation logic for subdomain creation / management.
    // We pass `false` as last argument because even if we don't have create
...
/// 3. Validates that the parent can create subdomains (based on the on-chain
/// setup). [all 2nd level names with valid tld can create names]
...
///     2.3 Validates that this subdomain can indeed be registered by that
/// parent.
///     2.4 Validates that the subdomain's expiration timestamp is less or equal
/// to the parents.
///     2.5 Checks if this subdomain already exists. [If it does, it aborts if
/// it's not expired, overrides otherwise]
///
/// It then saves the configuration for that child (manage-able by the parent),
/// and returns the SuinsRegistration object.
public fun new(
    suins: &mut SuiNS,
    parent: &SuinsRegistration,
    clock: &Clock,
    ...
    // validate that the requested expiration timestamp is not greater than the
    // parent's one.
    assert!(
        expiration_timestamp_ms <= parent.expiration_timestamp_ms(),
        EInvalidExpirationDate,
    );

    // We register the subdomain (e.g. `subdomain.example.sui`) and return the
    // SuinsRegistration object.
    // Aborts with `suins::registry::ERecordExists` if the subdomain already
    // exists.
    ...
    // We save a setting like: `subdomain.example.sui` -> { allow_creation:
    // true/false, allow_time_extension: true/false }
    if (allow_creation) {
        internal_set_flag(
            suins,
            subdomain,
...
public fun extend_expiration(
    ...
    // we need to make sure this name record exists (both child + parent),
    // otherwise we don't have a valid object.
    assert!(
    ...
    // (to prevent cases where owner of the parent changed. When that happens,
    // subdomains lose all abilities to renew / create subdomains)
    assert!(
        parent(nft) == option::borrow(&parent_name_record).nft_id(),
        EParentChanged,
    );

    // validate that expiration date is > than the current.
    assert!(
        expiration_timestamp_ms > nft.expiration_timestamp_ms(),
        EInvalidExpirationDate,
    );
    // validate that the requested expiration timestamp is not greater than the
    // parent's one.
    ...
    registry_mut(suins).set_expiration_timestamp_ms(
        nft,
        subdomain,
        expiration_timestamp_ms,
    );
...
/// --> For creations: A parent can't retract already created children, nor can
/// limit the depth if creation capability is on.
public fun edit_setup(
    ...
    config::assert_is_valid_subdomain(
        &parent_domain,
        &subdomain,
        app_config(suins),
    );
...
// Sets/removes a (key,value) on the domain's NameRecord metadata (depending on
// cases).
...
/// Does all the regular checks for validating that a parent `SuinsRegistration`
/// object
/// can operate on a given subdomain.
///
/// 1. Checks that NFT is authorized.
/// 2. Checks that the parent can create subdomains (applies to subdomain `node`
/// names).
/// 3. Validates that the subdomain is valid (accepted TLD, depth, length, is
/// child of given parent, etc).
fun internal_validate_nft_can_manage_subdomain(
    suins: &SuiNS,
    parent: &SuinsRegistration,
    clock: &Clock,
    subdomain: Domain,
    // Set to `true` for `validate_creation` if you want to validate that the
    // parent can create subdomains.
    // Set to false when editing the setup of a subdomain or removing leaf
    // names.
...
/// Validate whether a `SuinsRegistration` object is eligible for creating a
/// subdomain.
/// 1. If the NFT is authorized (not expired, active)
/// 2. If the parent is a subdomain, check whether it is allowed to create
/// subdomains.
fun internal_assert_parent_can_create_subdomains(self: &SuiNS, parent: Domain) {
    ...
    // if `parent` is a subdomain. We check the subdomain config to see if we
    // are allowed to mint subdomains.
    // For regular names (e.g. example.sui), we can always mint subdomains.
    // if there's no config for this parent, and the parent is a subdomain, we
    // can't create deeper names.
    assert!(
        is_creation_allowed(&record_metadata(self, parent)),
        ECreationDisabledForSubDomain,
    );
}

/// An internal function to add a subdomain to the registry with the correct
/// expiration timestamp.
/// It doesn't check whether the expiration is valid. This needs to be checked
...
fun internal_create_subdomain(
    ...
    let mut nft = registry.add_record_ignoring_grace_period(
        subdomain,
        1,
        clock,
        ctx,
    ...
    // but we can correct it easily here.
    ...
    // attach the `ParentID` to the SuinsRegistration, so we validate that the
    // parent who created this subdomain
