...
/// Domain user tries to purchase has a size missmatch. Only applicable for 3 +
/// 4 length domains.
const ESizeMissMatch: u64 = 2;

/// We have a requirement that this promotion will run for a specified amount of
/// time (30 Days).
/// I believe it's better to deauthorize the app when we do not want to have it
/// any more,
...
public fun claim(
    ...
    // We understand if a domain was bought in an auction if the expiry date is
    // less than the last day of auction + 1 year.
    assert!(
    ...
    // if size < 5, we need to make sure we're getting a domain name of the same
    // size.
    assert!(
        !(
    ...
    let registry = suins::app_registry_mut<BogoApp, Registry>(
        BogoApp {},
        suins,
    );
    let mut nft = registry.add_record(new_domain, DEFAULT_DURATION, clock, ctx);

    // mark both the new and the current domain presented as used, so that they
    // can't
