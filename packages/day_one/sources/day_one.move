...
/// This module defines the `DayOne` Object airdropped to early supporters of
/// the SuiNS project.
...
fun init(otw: DAY_ONE, ctx: &mut TxContext) {
    ...
    // For SuiNS, we need 1 SetupCap to manage all the required addresses. We'll
    // be setting up around 75K addresses.
...
public fun mint(
    ...
    // fails if not found.
    let lookup = df::remove_if_exists(
        &mut self.id,
