...
#[test]
fun test_e2e() {
    // an e2e scenario were we just purchase 3 domains normally using 3
    // registered ones.
    let mut scenario_val = test_init();
    let scenario = &mut scenario_val;
    scenario.next_tx(USER_ADDRESS);
    let clock = scenario.take_shared<Clock>();
    let (mut domain1, mut domain2, mut domain3, mut day_one) = prepare(
        ctx(scenario),
        &clock,
    );
    let mut suins = scenario.take_shared<SuiNS>();
...
fun failure_test_domain_already_used() {
    ...
    scenario.next_tx(USER_ADDRESS);
    let clock = scenario.take_shared<Clock>();
    let (mut domain1, domain2, domain3, mut day_one) = prepare(
        ctx(scenario),
        &clock,
    );
    let mut suins = scenario.take_shared<SuiNS>();
...
fun failure_test_domain_not_bought_in_auction() {
    ...
    let (domain1, domain2, domain3, mut day_one) = prepare(
        ctx(scenario),
        &clock,
    );
    let mut suins = scenario.take_shared<SuiNS>();
...
fun failure_test_length_missmatch_2() {
    ...
    scenario.next_tx(USER_ADDRESS);
    let clock = scenario.take_shared<Clock>();
    let (domain1, mut domain2, domain3, mut day_one) = prepare(
        ctx(scenario),
        &clock,
    );
    let mut suins = scenario.take_shared<SuiNS>();
...
fun failure_test_length_missmatch_3() {
    ...
    scenario.next_tx(USER_ADDRESS);
    let clock = scenario.take_shared<Clock>();
    let (domain1, domain2, mut domain3, mut day_one) = prepare(
        ctx(scenario),
        &clock,
    );
    let mut suins = scenario.take_shared<SuiNS>();
