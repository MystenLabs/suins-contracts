...
module coupons::coupon_tests {
    ...
    fun test_e2e() {
        ...
        register_with_coupon(b"5_SUI_DISCOUNT".to_string(), b"test.sui".to_string(), 1, 195 * mist_per_sui(), 0, user(), scenario);

        // original price would be 400 (200*2 years). 25% discount should bring it down to 300.
        register_with_coupon(b"25_PERCENT_DISCOUNT_MAX_2_YEARS".to_string(), b"jest.sui".to_string(), 2, 300 * mist_per_sui(), 0, user(), scenario);

        // Test that this user-specific coupon works as expected
        register_with_coupon(b"25_PERCENT_DISCOUNT_USER_ONLY".to_string(), b"fest.sui".to_string(), 2, 300 * mist_per_sui(), 0, user(), scenario);

        // 50% discount only on names 5+ digits
        register_with_coupon(b"50_PERCENT_5_PLUS_NAMES".to_string(), b"testo.sui".to_string(), 1, 25 * mist_per_sui(), 0, user(), scenario);

        // 50% discount only on names 3 digit names.
        register_with_coupon(b"50_PERCENT_3_DIGITS".to_string(), b"tes.sui".to_string(), 1, 600 * mist_per_sui(), 0, user(), scenario);
    ...
    fun test_price_calculation(){
        ...
        populate_coupons(scenario);
        {
            ...
            let sale_price = coupon_house::calculate_sale_price(&suins, 100, b"50_PERCENT_5_PLUS_NAMES".to_string());
