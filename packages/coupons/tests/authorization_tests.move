...
module coupons::app_authorization_tests {
    ...
    use coupons::setup::{Self, UnauthorizedTestApp, TestApp};
    ...
    fun authorized_app_get_app_success(){
        ...
        let scenario = &mut scenario_val;
        {
            test_scenario::next_tx(scenario, setup::admin_address());

            let coupon_house = test_scenario::take_shared<CouponHouse>(scenario);
            let admin_cap = test_scenario::take_from_sender<AdminCap>(scenario);

            coupons::app_data_mut_as_admin(&admin_cap, &mut coupon_house);
