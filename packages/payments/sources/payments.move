...
/// `handle_payment`; only the price source differs.
public fun handle_payment_pro<T>(
    ...
    mut payment: Coin<T>,
    clock: &Clock,
    price_info_object: &ProPriceInfoObject,
    ...
    let target_currency_amount = calculate_price_pro<T>(
        suins,
...
/// 2. const price = calculate_price_pro<SUI>(suins, baseAmount, clock, priceInfoObject);
/// 3. const coin = txb.splitCoins(baseCoin, [price])
/// 4. handle_payment_pro<SUI>(suins, bbbVault, intent, coin, clock, priceInfoObject, userPriceGuard);
///
/// `calculate_price` variant that reads the Pro-compatible Pyth feed, for use
/// after the Pyth Core to Pro cutover. Behaviour matches the pre-cutover
/// `calculate_price`; only the price source differs.
public fun calculate_price_pro<T>(
    ...
    let price = pyth_pro::get_price_no_older_than(
        price_info_object,
        clock,
        config.max_age,
    );
